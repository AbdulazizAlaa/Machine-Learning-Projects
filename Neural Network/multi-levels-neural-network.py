...
Wlj = shared(np.random.rand(numOutputNeurons, numHiddenNeurons2)*0.01)
...
Alj = T.nnet.softmax(T.dot(Wlj, Akj))

# E = T.sum(T.sub(t, Akj.T)**2)/2
E = T.mean(T.nnet.categorical_crossentropy(Alj.T, t))
...
O = T.nnet.softmax(T.dot(Wlj, T.nnet.sigmoid(T.dot(Wkj, T.nnet.sigmoid(T.dot(Wji, X.T))))))
